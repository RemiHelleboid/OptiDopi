...
struct SimulatedAnnealHistory {
    ...
    void export_to_csv(const std::string& filename) {
        ...
        for (std::size_t i = 0; i < solutions.size(); ++i) {
            ...
            fmt::print(file, "{:e}", solutions[i][0]);
            file << std::endl;
        }
    }
};
...
class SimulatedAnnealing {
 private:
    ...
    /**
     * @brief Neighbour function, returns a new solution based on the current solution.
     * First argument is the variables vector.
     *
     */
    std::function<std::vector<double>(std::vector<double>)> m_neighbour_function;
 ...
 public:
    ...
    std::vector<double> clip_variables(const std::vector<double>& variables);
